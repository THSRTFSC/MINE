...
public class LoginPresenter implements LoginContract.Presenter, Callback {
        ...
        switch (inputType) {
            case LoginContract.INPUT_TYPE_USER_ACCOUNT:
                if (valueLength < MAX_LENGTH_USER_ACCOUNT) {
                    Toast.makeText(MINEApplication.getInstance(), R.string.account_not_longer_enough, Toast.LENGTH_LONG).show();
                    return false;
                }
                break;
            case LoginContract.INPUT_TYPE_USER_PASSWORD:
                if (MIN_LENGTH_PASSWORD >= valueLength || valueLength >= MAX_LENGTH_PASSWORD) {
                    Toast.makeText(MINEApplication.getInstance(), R.string.password_not_longer_enough, Toast.LENGTH_LONG).show();
                    return false;
                }
                break;
        }
    ...
    public void onFailure(Call call, IOException e) {
        ...
        Toast.makeText(MINEApplication.getInstance(), R.string.login_fail, Toast.LENGTH_LONG).show();
    }
    ...
    public void onResponse(Call call, Response response) throws IOException {
        ...
        try {
            ...
            MINEApplication.getInstance().setLoginToken(loginToken);
